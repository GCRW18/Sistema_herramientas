...
import { Subject, takeUntil, debounceTime, distinctUntilChanged, finalize, of } from 'rxjs';
...
export class PrestamoTercerosComponent implements OnInit, OnDestroy {
    ...
    private _setupTecnicoSearch(): void {
        this._tecnicoSearch$.pipe(
            ...
            this.tecnicosFiltrados = res || [];
            this.showTecnicoDropdown = (res || []).length > 0;
        });
    }

    onTecnicoInput(v: string): void {
        this.internalForm.patchValue({ buscarTecnico: v }, { emitEvent: false });
        if (v.length >= 2) this._tecnicoSearch$.next(v);
        else this.showTecnicoDropdown = false;
    }
