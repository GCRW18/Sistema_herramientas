...
import { Subject, takeUntil, finalize, debounceTime, distinctUntilChanged, forkJoin, of } from 'rxjs';
...
export class DevolucionHerramientaComponent implements OnInit, OnDestroy {
    ...
    _funcionarioSearch$ = new Subject<string>();
    funcionariosFiltrados: { id: string; nombre: string; cargo: string; licencia: string }[] = [];
    ...
    todasLasHerramientas: { id: number; codigo: string; nombre: string; pn: string; sn: string }[] = [];
    herramientasFiltradas: { id: number; codigo: string; nombre: string; pn: string; sn: string }[] = [];
    ...
    private _setupFuncionarioSearch(): void {
        this._funcionarioSearch$.pipe(
            ...
            this.funcionariosFiltrados = res || [];
            this.showFuncionarioDropdown = this.funcionariosFiltrados.length > 0;
        });
    }
    ...
    selectFuncionario(f: { id: string; nombre: string; cargo: string; licencia: string }): void {
        this._funcionarioNombre = f.nombre;
        this._funcionarioLicencia = f.licencia;
        this.devolucionForm.patchValue({ funcionario: f.nombre });
    ...
    private _cargarHerramientas(): void {
        ...
        // Carga todas las herramientas (sin filtro de status) para incluir las prestadas
        this.movementService.getHerramientasDisponibles({}).pipe(
            takeUntil(this._unsubscribeAll),
            finalize(() => this.herramientaLoading = false),
            catchError(() => of([]))
        ).subscribe((tools: any[]) => {
    ...
    onHerramientaInput(val: string): void {
        ...
        if (term.length < 2) {
            ...
            this.showHerramientaDropdown = false;
            return;
        }
    ...
    selectHerramienta(h: { id: number; codigo: string; nombre: string; pn: string; sn: string }): void {
        this._herramientaSeleccionada = { codigo: h.codigo, nombre: h.nombre };
        this.devolucionForm.patchValue({ codigoHerramienta: h.codigo });
        this.herramientasFiltradas = [];
        this.showHerramientaDropdown = false;
    }

    clearHerramienta(): void {
        this._herramientaSeleccionada = null;
        this.devolucionForm.patchValue({ codigoHerramienta: '' });
        this.herramientasFiltradas = [];
        this.showHerramientaDropdown = false;
    }

    hideHerramientaDropdown(): void {
        setTimeout(() => this.showHerramientaDropdown = false, 150);
    }
    ...
    abrirModalConfirmacion(): void {
        ...
        if (!this._funcionarioNombre) {
            this.showMessage('Debe buscar y seleccionar un funcionario primero', 'error');
    ...
    isBusquedaValida(): boolean {
        ...
        return !!f.funcionario && !!f.responsableRecibe;
    }

    realizarConsulta(): void {
        const nombreFuncionario = this._funcionarioNombre || this.devolucionForm.get('funcionario')?.value || '';
        ...
        const nombreSafe = nombreFuncionario.replace(/'/g, "''");
        const licenciaSafe = this._funcionarioLicencia.replace(/'/g, "''");
        ...
        forkJoin({
            ...
            next: ({ loans, items }) => {
                if (!loans || loans.length === 0) {
                    this.showMessage(`No hay préstamos activos para ${nombreFuncionario}`, 'info');
    ...
    getCondicionIcon(condicion: CondicionDevolucion): string {
        return this.condiciones.find(c => c.value === condicion)?.icon || 'help_outline';
    }
    ...
    finalizar(): void {
        ...
        const nombreFuncionario = this.devolucionForm.get('funcionario')?.value || '';
        const selectedItems = this.getSelectedItems();
        ...
        this.movementService.registrarDevolucionPrestamo({
            ...
            requested_by_name: nombreFuncionario,
            responsible_person: this.devolucionForm.value.responsableRecibe,
            recipient: nombreFuncionario,
