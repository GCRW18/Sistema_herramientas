...
export class EnvioOtrasBasesComponent implements OnInit, OnDestroy {
    ...
    abrirModalDatosEnvio(): void {
        this.dialogRefActual = this.dialog.open(this.datosEnvioModal, {
            width: '860px', maxWidth: '95vw', panelClass: 'no-padding-dialog', disableClose: true
        });
    }
    ...
    async openHerramientaAEnviar(): Promise<void> {
        ...
        const dialogRef = this.dialog.open(HerramientaAEnviarComponent, {
            ...
            data: { mode: 'add', tipoEnvio: this.exitForm.get('tipoEnvio')?.value, prioridad: this.exitForm.get('prioridad')?.value, baseDestino: this.exitForm.get('baseDestino')?.value }
        });
    ...
    async openHerramientaAEnviarEdit(item: ExitItem): Promise<void> {
        const { HerramientaAEnviarComponent } = await import('./herramienta-a-enviar/herramienta-a-enviar.component');
        const dialogRef = this.dialog.open(HerramientaAEnviarComponent, {
            width: '800px', maxWidth: '95vw', height: 'auto', maxHeight: '90vh',
            panelClass: 'no-padding-dialog', disableClose: false,
        ...
        dialogRef.afterClosed().subscribe(result => {
            if (result?.action === 'editar') {
                const updatedItems = this.dataSource().map(i => {
                    if (i.id !== item.id) return i;
                    return {
                        ...i,
                        toolId:        result.data.toolId ?? result.data.id_tool ?? i.toolId,
                        codigo:        result.data.codigo        || i.codigo,
                        descripcion:   result.data.nombre || result.data.descripcion || i.descripcion,
                        marca:         result.data.marca         || i.marca,
                        partNumber:    result.data.pn || result.data.part_number    || i.partNumber,
                        serialNumber:  result.data.sn || result.data.serial_number  || i.serialNumber,
                        cantidad:      result.data.cantidad       ?? i.cantidad,
                        um:            result.data.unidad || result.data.unit_of_measure || i.um,
                        estado:        result.data.estadoFisico   || i.estado,
                        prioridad:     result.data.prioridad      || i.prioridad,
                        motivo:        result.data.motivo         || i.motivo,
                        observaciones: result.data.observacion    || i.observaciones
                    };
                });
                this.dataSource.set(updatedItems);
                this.showMessage(`Herramienta ${item.codigo} actualizada`, 'success');
            }
        });
    }
    ...
    getMotivoLabel(motivo: string): string {
        const labels: Record<string, string> = {
            'APOYO_OPERACIONAL':          'Apoyo Op.',
            'MANTENIMIENTO_PROGRAMADO':   'Mant. Prog.',
            'MANTENIMIENTO_NO_PROGRAMADO':'Mant. No Prog.',
            'AOG_EMERGENCIA':             'AOG',
            'TRASPASO_DEFINITIVO':        'Traspaso',
            'CALIBRACION_EXTERNA':        'Calibración',
            'REPARACION':                 'Reparación',
            'PRESTAMO_TEMPORAL':          'Préstamo',
