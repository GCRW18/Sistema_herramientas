...
import { takeUntil, debounceTime, finalize, switchMap, map, catchError, distinctUntilChanged } from 'rxjs/operators';
...
export class AjusteIngresoComponent implements OnInit, OnDestroy {
    ...
    private _setupFuncionarioSearch(
        ...
        setList: (res: { id: string; nombre: string; cargo: string }[]) => void,
        ...
        ).subscribe(res => { setList(res || []); setDropdown((res || []).length > 0); });
    }
    ...
    hideRealizadoPorDropdown(): void {
        setTimeout(() => this.showRealizadoPorDropdown = false, 150);
    }
    ...
    hideAprobadoPorDropdown(): void {
        setTimeout(() => this.showAprobadoPorDropdown = false, 150);
    }
