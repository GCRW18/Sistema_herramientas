...
import { Subject, takeUntil, finalize, timeout, debounceTime, distinctUntilChanged, of } from 'rxjs';
...
export class TraspasoOtraAreaComponent implements OnInit, OnDestroy {
    ...
    private _setupPersonaSearch(): void {
        this._personaSearch$.pipe(
            ...
            this.showPersonaDropdown = (res || []).length > 0;
        });
    }

    onPersonaInput(v: string): void {
        this.transferForm.patchValue({ nombreCompletoInput: v }, { emitEvent: false });
