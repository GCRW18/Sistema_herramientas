    ...
    <div class="flex flex-col w-full max-h-[88vh] bg-[#f8f9fc] dark:bg-slate-900 border-[3px] border-black rounded-2xl overflow-hidden font-sans shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
        ...
        <div class="bg-[#0F172AFF] text-white px-3.5 sm:px-5 py-3 flex items-center gap-3 border-b-2 border-black shrink-0">
            <div class="w-10 h-10 sm:w-12 sm:h-12 shrink-0 bg-amber-400 text-black border-2 border-black rounded-lg flex items-center justify-center font-black text-sm sm:text-base shadow-[2px_2px_0px_0px_#000]">
                ...
                <ng-template #floorIcon><mat-icon class="!text-[22px] text-black">foundation</mat-icon></ng-template>
            </div>
            <div class="flex-1 min-w-0">
                <h2 class="text-sm sm:text-base font-black uppercase tracking-tight leading-none truncate">
                    {{ level.isFloor ? 'Nivel Suelo' : ('Nivel ' + level.numero) }} · <span class="text-amber-400">{{ level.nombre }}</span>
                </h2>
                <p class="text-[10px] sm:text-[11px] text-gray-300 font-bold uppercase mt-1 truncate">
                    {{ data.almacen.codigo }} · Estante {{ rack.nombre }} · <span class="font-mono">{{ level.codigo }}</span>
                </p>
            </div>
            <span class="hidden sm:flex px-2.5 py-1 text-[11px] font-black uppercase border-2 border-black rounded-lg bg-white text-black items-center gap-1 shrink-0 shadow-[2px_2px_0px_0px_#000]">
                <mat-icon class="!text-sm !w-4 !h-4">handyman</mat-icon>
                {{ herramientas.length }}
            </span>
            <span *ngIf="kits.length" class="hidden sm:flex px-2.5 py-1 text-[11px] font-black uppercase border-2 border-black rounded-lg bg-blue-100 text-blue-800 items-center gap-1 shrink-0 shadow-[2px_2px_0px_0px_#000]">
                <mat-icon class="!text-sm !w-4 !h-4">inventory_2</mat-icon>
                {{ kits.length }}
            </span>
            <span *ngIf="miscelaneos.length" class="hidden sm:flex px-2.5 py-1 text-[11px] font-black uppercase border-2 border-black rounded-lg bg-purple-100 text-purple-800 items-center gap-1 shrink-0 shadow-[2px_2px_0px_0px_#000]">
                <mat-icon class="!text-sm !w-4 !h-4">category</mat-icon>
                {{ miscelaneos.length }}
            </span>
            <button (click)="cerrar()" matTooltip="Cerrar"
                    class="w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center bg-[#FF1414FF] border-2 border-black rounded-lg shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none transition-all shrink-0">
                <mat-icon class="text-white !text-lg">close</mat-icon>
            </button>
        </div>

        <!-- Toolbar -->
        <div class="px-3.5 sm:px-5 py-2.5 bg-white dark:bg-slate-800 border-b-2 border-black flex items-center gap-2.5 shrink-0">
            <div class="relative flex-1 min-w-0">
                <input type="text" [formControl]="filtroControl" placeholder="Buscar en este nivel..."
                       class="w-full h-10 pl-9 pr-3 bg-[#f8f9fc] dark:bg-slate-900 text-black dark:text-white border-2 border-black rounded-lg font-bold text-[13px] focus:outline-none focus:shadow-[2px_2px_0px_0px_#000] transition-all">
                <mat-icon class="absolute left-2.5 top-2 text-black dark:text-white !text-lg">search</mat-icon>
            </div>
            <button *appHasPermission="'inv_ubicaciones.manage'" (click)="agregarHerramienta()"
                    class="shrink-0 px-3.5 py-2 bg-amber-400 text-black font-black text-[11px] border-2 border-black rounded-lg shadow-[2px_2px_0px_0px_#000] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none transition-all uppercase flex items-center gap-1.5">
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-3.5 sm:p-5 min-h-[260px]">
            <div class="flex flex-col gap-3">
                ...
                <div class="flex flex-col gap-2.5">
                    <div *ngFor="let t of herramientasFiltradas"
                         ...
                         class="group relative bg-white dark:bg-slate-800 border-2 border-black rounded-xl p-2.5 sm:p-3.5 shadow-[3px_3px_0px_0px_#000] hover:-translate-y-[1px] hover:shadow-[4px_4px_0px_0px_#000] transition-all flex items-center gap-2.5">

                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-1.5 mb-0.5 flex-wrap">
                                <span class="font-mono text-[10px] font-black px-1.5 py-0.5 bg-gray-200 dark:bg-slate-700 border border-black rounded text-black dark:text-white">
                                    {{ t.codigo }}
                                </span>
                                <span [ngClass]="[estadoBadge(t.estado).bg, estadoBadge(t.estado).tx]"
                                      class="px-1.5 py-0.5 text-[9px] font-black uppercase border border-black rounded">
                                    {{ t.estado }}
                                </span>
                            </div>
                            <p class="font-black text-[12px] sm:text-[13px] leading-tight text-black dark:text-white truncate mt-0.5">{{ t.nombre }}</p>
                            <p class="text-[10px] sm:text-[11px] font-bold text-gray-500 truncate mt-0.5">
                        ...
                        <div class="text-right shrink-0 px-2.5 sm:px-3 border-r-2 border-gray-200 dark:border-slate-700">
                            <p class="text-base sm:text-xl font-black text-black dark:text-white leading-none">{{ t.cantidad }}</p>
                            <p class="text-[9px] font-bold text-gray-500 uppercase mt-0.5">{{ t.um }}</p>
                        </div>
                        ...
                        <div class="flex items-center justify-center gap-1 shrink-0">
                            <button *appHasPermission="'inv_ubicaciones.manage'" (click)="editarHerramienta(t)" matTooltip="Editar"
                                    class="w-8 h-8 flex items-center justify-center bg-amber-400 border-2 border-black rounded-lg shadow-[2px_2px_0px_0px_#000] hover:translate-y-[1px] hover:shadow-none transition-all">
                                <mat-icon class="!text-base text-black">edit</mat-icon>
                            </button>
                            <button *appHasPermission="'inv_ubicaciones.move'" (click)="moverHerramienta(t)" matTooltip="Mover"
                                    class="w-8 h-8 flex items-center justify-center bg-blue-500 border-2 border-black rounded-lg shadow-[2px_2px_0px_0px_#000] hover:translate-y-[1px] hover:shadow-none transition-all">
                                <mat-icon class="!text-base text-white">swap_horiz</mat-icon>
                            </button>
                            <button *appHasPermission="'inv_ubicaciones.manage'" (click)="quitarHerramienta(t)" matTooltip="Quitar del nivel"
                                    class="w-8 h-8 flex items-center justify-center bg-[#FF1414FF] border-2 border-black rounded-lg shadow-[2px_2px_0px_0px_#000] hover:translate-y-[1px] hover:shadow-none transition-all">
                                <mat-icon class="!text-base text-white">remove_circle</mat-icon>
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div *ngIf="!herramientasFiltradas.length"
                     class="text-center py-12 bg-white dark:bg-slate-800 border-2 border-dashed border-gray-400 dark:border-gray-600 rounded-xl">
                    <mat-icon class="!text-5xl text-gray-300 dark:text-gray-600">handyman</mat-icon>
                    <p class="text-[13px] font-black uppercase text-gray-500 mt-3">
                    ...
                    <ng-container *appHasPermission="'inv_ubicaciones.manage'">
                        <button *ngIf="herramientas.length === 0" (click)="agregarHerramienta()"
                                class="mt-4 px-4 py-2 bg-amber-400 text-black font-black text-[11px] border-2 border-black rounded-lg shadow-[2px_2px_0px_0px_#000] hover:translate-y-[1px] hover:shadow-none transition-all uppercase inline-flex items-center gap-1.5">
                            <mat-icon class="!text-base text-black">add</mat-icon>
                            Agregar la primera
                        </button>
                    </ng-container>
                ...
                <ng-container *ngIf="kitsFiltrados.length">
                    <div class="flex items-center gap-2 mt-1">
                        <mat-icon class="!text-base !w-4 !h-4 text-blue-700">inventory_2</mat-icon>
                        <span class="text-[11px] font-black uppercase text-gray-500">Kits en este nivel</span>
                    </div>

                    <div class="flex flex-col gap-2.5">
                        <div *ngFor="let k of kitsFiltrados"
                             class="group relative bg-blue-50 dark:bg-slate-800 border-2 border-black rounded-xl p-2.5 sm:p-3.5 shadow-[3px_3px_0px_0px_#000] hover:-translate-y-[1px] hover:shadow-[4px_4px_0px_0px_#000] transition-all flex items-center gap-2.5">

                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-1.5 mb-0.5 flex-wrap">
                                    <span class="font-mono text-[10px] font-black px-1.5 py-0.5 bg-blue-200 dark:bg-slate-700 border border-black rounded text-black dark:text-white">
                                        {{ k.codigo }}
                                    </span>
                                    <span class="px-1.5 py-0.5 text-[9px] font-black uppercase border border-black rounded"
                                          [ngClass]="k.isComplete ? 'bg-green-500 text-white' : 'bg-[#FF1414FF] text-white'">
                                        {{ k.isComplete ? 'COMPLETO' : 'INCOMPLETO' }}
                                    </span>
                                </div>
                                <p class="font-black text-[12px] sm:text-[13px] leading-tight text-black dark:text-white truncate mt-0.5">{{ k.nombre }}</p>
                                <p class="text-[10px] sm:text-[11px] font-bold text-gray-500 truncate mt-0.5">
                ...
                <ng-container *ngIf="miscelaneosFiltrados.length">
                    <div class="flex items-center gap-2 mt-1">
                        <mat-icon class="!text-base !w-4 !h-4 text-purple-700">category</mat-icon>
                        <span class="text-[11px] font-black uppercase text-gray-500">Misceláneos en este nivel</span>
                    </div>

                    <div class="flex flex-col gap-2.5">
                        <div *ngFor="let m of miscelaneosFiltrados"
                             class="group relative bg-purple-50 dark:bg-slate-800 border-2 border-black rounded-xl p-2.5 sm:p-3.5 shadow-[3px_3px_0px_0px_#000] hover:-translate-y-[1px] hover:shadow-[4px_4px_0px_0px_#000] transition-all flex items-center gap-2.5">

                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-1.5 mb-0.5 flex-wrap">
                                    <span class="font-mono text-[10px] font-black px-1.5 py-0.5 bg-purple-200 dark:bg-slate-700 border border-black rounded text-black dark:text-white">
                                        {{ m.codigo }}
                                    </span>
                                    <span *ngIf="m.itemType" class="px-1.5 py-0.5 text-[9px] font-black uppercase border border-black rounded bg-gray-200 text-black">
                                        {{ m.itemType }}
                                    </span>
                                </div>
                                <p class="font-black text-[12px] sm:text-[13px] leading-tight text-black dark:text-white truncate mt-0.5">{{ m.nombre }}</p>
                            </div>

                            <div class="text-right shrink-0 px-2.5 sm:px-3 border-r-2 border-gray-200 dark:border-slate-700">
                                <p class="text-base sm:text-xl font-black text-black dark:text-white leading-none">{{ m.quantityInStock }}</p>
                                <p class="text-[9px] font-bold text-gray-500 uppercase mt-0.5">{{ m.unitOfMeasure }}</p>
                            </div>
