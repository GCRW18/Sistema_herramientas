...
interface KitCategory {
    id_kit_category: number;
    name: string;
    active: boolean;
}

@Component({
    selector: 'app-categoria-manager-dialog',
    standalone: true,
        ...
        <div>
            ...
            <div *ngIf="!loading()" class="flex flex-wrap gap-1.5 min-h-[36px]">
                <div *ngFor="let cat of categorias()"
                     ...
                     [ngClass]="confirmDeleteId() === cat.id_kit_category ? 'bg-red-600' : getCategoriaClass(cat.name)">

                    <!-- Estado normal -->
                    <ng-container *ngIf="confirmDeleteId() !== cat.id_kit_category">
                        <span>{{ cat.name }}</span>
                        <button (click)="pedirConfirmacion(cat)"
                                [disabled]="deletingId() === cat.id_kit_category"
                    ...
                    <ng-container *ngIf="confirmDeleteId() === cat.id_kit_category">
                        ...
                        <button (click)="confirmarEliminar(cat)"
                            ...
                            <span *ngIf="deletingId() === cat.id_kit_category"
                                  class="w-2.5 h-2.5 border border-white border-t-transparent rounded-full animate-spin"></span>
                            <mat-icon *ngIf="deletingId() !== cat.id_kit_category" class="!text-[10px] !w-3 !h-3 text-white">check</mat-icon>
                        </button>
...
export class CategoriaManagerDialogComponent implements OnInit {

    private dialogRef   = inject(MatDialogRef<CategoriaManagerDialogComponent>);
    private kitsService = inject(KitsService);

    categorias       = signal<{ id_kit_category: number; name: string; active: boolean }[]>([]);
    ...
    cargar(): void {
        ...
        this.kitsService.getKitCategories().pipe(
            ...
            this.categorias.set(cats.filter(c => c.active));
        });
    }
    ...
    agregarCategoria(): void {
        ...
        this.errorMsg.set('');
        this.kitsService.createKitCategory(name).subscribe({
            next: () => {
    ...
    pedirConfirmacion(cat: KitCategory): void {
        this.errorMsg.set('');
        this.confirmDeleteId.set(cat.id_kit_category);
    }
    ...
    confirmarEliminar(cat: KitCategory): void {
        this.deletingId.set(cat.id_kit_category);
        this.errorMsg.set('');
        this.kitsService.deleteKitCategory(cat.id_kit_category).subscribe({
            next: () => {
                this.deletingId.set(null);
                this.confirmDeleteId.set(null);
                this.categorias.update(list => list.filter(c => c.id_kit_category !== cat.id_kit_category));
            },
            error: () => {
                this.deletingId.set(null);
                this.confirmDeleteId.set(null);
